...
import { fetchIncomeAndExpenditureObligations } from "@/lib/hmrc-obligations";
...
export async function GET(request: Request) {
  ...
  if (!taxIds.niNumber) {
    return NextResponse.json(
      { error: "Add your National Insurance number on the dashboard before fetching obligations." },
      { status: 400 },
    );
  }
  ...
  const result = await fetchIncomeAndExpenditureObligations({
    userId,
    request,
